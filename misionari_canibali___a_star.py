...
class Graf:
    ...
    def fct_euristica_h2(self, nod_info):
        ...
        if nod_info[1] + nod_info[2] !=0 and nod_info[0] == 'est':
            x = 2*((nod_info[1] + nod_info[2]-2)/Nod.NR_LOCURI_BARCA-1)+1
        elif nod_info[1] + nod_info[2] !=0 and nod_info[0] == 'vest':
            x = 2 * ((nod_info[1] + nod_info[2] - 2) / Nod.NR_LOCURI_BARCA - 1) + 2
        return x
    ...
    def muta_barca(self, nod_info, nr_mis, nr_can):
        ...
        if nod_info[0] == 'est':
            nod_copy[0] = 'vest'
            nod_copy[1] = nod_copy[1] - nr_mis
            nod_copy[2] = nod_copy[2] - nr_can
            nod_copy[3] = nod_copy[3] + nr_mis
            nod_copy[4] = nod_copy[4] + nr_can
        else:
            nod_copy[0] = 'est'
            nod_copy[1] = nod_copy[1] + nr_mis
            nod_copy[2] = nod_copy[2] + nr_can
            nod_copy[3] = nod_copy[3] - nr_mis
            nod_copy[4] = nod_copy[4] - nr_can
        if (nod_copy[1] >= nod_copy[2] or nod_copy[1] == 0) and (nod_copy[3] >= nod_copy[4] or nod_copy[3] == 0):
            return nod_copy
        else:
            return None
    ...
    def calculeaza_succesori(self, nod):
        ...
        for f1 in range(0,Nod.NR_LOCURI_BARCA+1):
            for f2 in range(0,Nod.NR_LOCURI_BARCA+1):
                if (f1 >= f2 or f1 == 0) and f1 + f2 <=Nod.NR_LOCURI_BARCA and f1 + f2 >=1:
                    nod_copy = self.muta_barca(nod.info, f1, f2)
                    if nod_copy is not None:
                        l_succesori.append((Nod(nod_copy,self.calculeaza_h(nod_copy)),1))
        return l_succesori
